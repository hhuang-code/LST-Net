...
class S3DIS(Dataset):
    def __init__(self, path, sub_grid_size, weak_label_ratio, test_area_idx=5):
        self.path = path
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            if os.path.exists(weak_label_sub_file):
                ...
                raise NotImplementedError(
                    "run the data_prepare_s3dis.py to generate weak labels for raw and sub PC")
...
if __name__ == '__main__':
    ...
    test_area_idx = 5
    dataset = S3DIS(path, sub_grid_size, weak_label_ratio, test_area_idx)
    ...
    split = 'training'
    train_dataset = IterableS3DIS(dataset, num_points, loop=loop, noise_init=noise_init, split=split)
    ...
    split = 'validation'
    valid_dataset = IterableS3DIS(dataset, num_points, loop=loop, noise_init=noise_init, split=split)
