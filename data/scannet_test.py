...
class SCANNET(Dataset):
    def __init__(self, path, sub_grid_size, weak_label_ratio):
        ...
        with open(os.path.join(self.path, 'list', 'scannetv2_test.txt')) as f:
    ...
    def load_sub_sampled_clouds(self, sub_grid_size):
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            sub_labels = np.zeros(sub_colors.shape[0]) - 1   # test set has no label, dummy label
            ...
            self.input_weak_labels[cloud_split] += [np.ones_like(sub_labels)]
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            if cloud_name in self.val_split:
                assert cloud_name == self.input_names['validation'][i]

                proj_file = join(tree_path, '{:s}_proj.pkl'.format(cloud_name))
                with open(proj_file, 'rb') as f:
                    # proj_idx, labels = pickle.load(f)
                    proj_idx = pickle.load(f)   # test set has no label
