...
def convert_pc2plyandweaklabels(anno_path, save_path, sub_pc_folder, weak_label_folder,
    ...
    Physically, each room will generate several files, including raw_pc.ply, sub_pc.ply, sub_pc.pkl (for the kdtree),
    proj_idx.pkl (for each raw point's nearest neighbor in the sub_pc)
    and weak labels for raw and sub_pc, respectively
    ...
    if not os.path.exists(sub_ply_file):
        # downsamlple ratio is around ~15
    ...
    # save projection indices for all raw points over the corresponding sub_pc
    ...
    if not os.path.exists(weak_label_sub_file):
        ...
        selected_idx = np.random.choice(num_sub_points, int(num_raw_points * weak_label_ratio), replace=True)
...
- output: 
  ...
  - 4) projection indices for each raw point over the sub_pc(used for DL validation/inference as the learning process 
  ONLY occurs on the sub_pc, therefore by relating raw point's relations to the sub_pc can help propagate their semantics.)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', type=str,
                        default=join(BASE_DIR, 'S3DIS/Stanford3dDataset_v1.2_Aligned_Version'), help='dataset path')
    ...
    for annotation_path in anno_paths:
        # e.g.: data/Stanford3dDataset_v1.2_Aligned_Version/Area_1/conferenceRoom_1/Annotations
