...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/s3dis_release.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/s3dis_release.yaml for all options', default=None,
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = Model(c=args.feat_dim, k=args.classes, lowrank=args.lowrank, sparse=args.sparse, vis_attn=args.vis_attn)
    if args.sync_bn:
    ...
    criterion = nn.CrossEntropyLoss(weight=torch.Tensor(DP.get_class_weights(args.data_name)).squeeze(),
                                    ignore_index=args.ignore_label).cuda()
    ...
    # load initial weight if applicable
    if args.weight:
    ...
    if args.data_name == 's3dis':
        ...
        dataset = SCANNET(path=args.data_root, sub_grid_size=args.voxel_size, weak_label_ratio=args.weak_label_ratio)
    else:
        raise NotImplementedError()
    ...
    if args.data_name == 's3dis':
        train_data = IterableS3DIS(dataset, num_points=args.point_max, loop=args.loop,
                                   noise_init=args.noise_init, split='training')
    elif args.data_name == 'scannet':
        train_data = IterableSCANNET(dataset, num_points=args.point_max, loop=args.loop,
                                     noise_init=args.noise_init, split='training')
    else:
        raise NotImplementedError()
    ...
    if args.evaluate:
        ...
        #                  voxel_max=args.voxel_max, transform=val_transform, weak_label_ratio=0.0)

        if args.data_name == 's3dis':
            val_data = IterableS3DIS(dataset, num_points=args.point_max, loop=args.loop,
                                     noise_init=args.noise_init, split='validation')
        elif args.data_name == 'scannet':
            val_data = IterableSCANNET(dataset, num_points=args.point_max, loop=args.loop,
                                       noise_init=args.noise_init, split='validation')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            if args.data_name == 's3dis' or args.data_name == 'scannet':
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    # for i, (coord, feat, target, mask, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n,), (n,), (b,)
    for i, (coord, feat, target, mask, _, _) in enumerate(train_loader):
        ...
        # coord, feat, target, mask = coord.cuda(), feat.cuda(), target.cuda(), mask.cuda()
        ...
        output = model([coord, feat, mask, offset])

        # if target.shape[-1] == 1:
        #     target = target[:, 0]  # for classification

        # target = target[mask.nonzero(as_tuple=True)[0]]
...
def validate(val_loader, model, criterion):
    ...
    for i, (coord, feat, target, mask, _, _) in enumerate(val_loader):
        data_time.update(time.time() - end)
